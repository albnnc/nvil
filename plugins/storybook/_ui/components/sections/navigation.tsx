...
export const Navigation = () => {
  const { storyDefs, activeStoryId } = useStorySummary() ?? {};
  const [query, setQuery] = useState("");
  const filteredStoryDefs = useMemo(() => {
    return (storyDefs ?? []).filter(
      (v) =>
  ...
  const filteredStoryGroups = useMemo(() => {
    return Array.from(
      new Set((filteredStoryDefs ?? []).map((v) => v.group))
    ...
    >
      {filteredStoryDefs.length ? (
        <div css={{ paddingTop: "0.5rem" }}>
          ...
          <Group storyDefs={filteredStoryDefs} activeStoryId={activeStoryId} />
        </div>
      ) : (
        <EmptyIcon
          width="1.65rem"
